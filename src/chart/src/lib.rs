...
use dominator::{Dom, text, class, stylesheet, html, svg, clone, events, with_node};
...
fn display_records(records: Vec<Record>, loading: Loading) -> Dom {
        ...
        svg!("svg", {
            ...
            .with_node!(element => {
                .global_event(clone!(state => move |e: events::Click| {
                    if state.hover_popup.get() {
                        ...
                        let x = (e.x() as f64) - 5.0;
                        // TODO use get_bounding_client_rect instead
                        let width: f64 = element.client_width() as f64;
            ...
            .children_signal_vec(state.information.signal_cloned().map(|information| {
                if let Some(information) = information {
                    ...
                    let d_bettors = vec!["M0,100".to_owned()];
                    let d_bet_amount = vec!["M0,100".to_owned()];
    ...
    fn make_info_popup(state: Rc<State>) -> Dom {
        ...
        html!("div", {
            ...
            .event(clone!(state => move |_: events::MouseEnter| {
                state.hover_popup.set_neq(true);
            }))

            .event(clone!(state => move |_: events::MouseLeave| {
    ...
    fn make_dropdown<A, S, F>(mutable: Mutable<A>, enabled: S, options: &[&'static str], mut f: F) -> Dom
        ...
        html!("select" => HtmlSelectElement, {
            ...
            .with_node!(element => {
                .event(clone!(mutable => move |_: events::Change| {
    ...
    {
        export_function("find_best", clone!(state, loading => closure!(move || {
        ...
        export_function("calculate_custom_strategy", clone!(state, loading => closure!(move |strategy: String| {
            loading.show();

            Timeout::new(0, clone!(loading, state => move || {
                match serde_json::from_str(&strategy) {
                    Ok(strategy) => {
                        console_log(salty_bet_bot::money(state.calculate_custom_strategy(strategy)));
                    },
                    Err(e) => {
        ...
        export_function("get_custom_strategy", clone!(state => closure!(move || {
            if let Some(strategy) = state.get_custom_strategy() {
                console_log(serde_json::to_string_pretty(&strategy).unwrap());
            }
        })));
    }
